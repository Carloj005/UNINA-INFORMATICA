...
- **Applicazione Regola `crediti.txt`:** Creati i file `crediti.txt` in tutte le cartelle `contenuto-non-proprio` (riconoscendo il contributo di Danilo Amalfitano per Analisi 1, Edoardo Capasso per Architettura, Hicortab/'L'informatica che appassiona' per Basi di Dati e POO, Armando Fiorillo per Sistemi Operativi e Linguaggi di Programmazione 1, Giuseppe Cautiero per Geometria e Prolog).
